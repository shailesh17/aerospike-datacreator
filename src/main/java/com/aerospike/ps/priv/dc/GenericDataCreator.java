...
import com.aerospike.ps.priv.dc.Config.Write;
import com.aerospike.ps.priv.dc.keygen.KeyGenerator;
import com.aerospike.ps.priv.dc.keygen.PipedKeyGenerator;
import com.aerospike.ps.priv.dc.util.HistoMeter;
import com.aerospike.ps.priv.dc.util.SizeHelper;
...
public class GenericDataCreator implements Listener {
	...
	public static HistoMeter OVERALL_WRITE_RATE;
	public static HistoMeter OVERALL_READ_RATE;
	...
	public void execute(File configFile) throws ParseException {
		...
		try {			
			...
			OVERALL_WRITE_RATE = new HistoMeter();
			metrics.register("\u2211 " + mId + ".wb/t", OVERALL_WRITE_RATE);	
			
			OVERALL_READ_RATE = new HistoMeter();
			metrics.register("\u2211 " + mId + ".rb/t", OVERALL_READ_RATE);	
			...
			Snapshot readRateBytes = null;
			Snapshot writeRateBytes = null;
			double writeRate = 0;
			double readRate = 0;
			...
			try {
				...
				readRateBytes = GenericDataCreator.OVERALL_READ_RATE.getSnapshot();
				writeRateBytes = GenericDataCreator.OVERALL_WRITE_RATE.getSnapshot();
				reporter.close();
			...
			System.out.println(
					String.format("\n\n********** Complete **********\n"
						...
						+ "Stored: %s (%s)\n"
						+ "Write Count: %s\n"
						+ "Write Avg. Latency: %s ms\n"
						+ "Write Min Latency: %s ms\n"
						+ "Write Max Latency: %s ms\n"
						+ "Write Records/Second: %s\n"
						+ "Write Bytes/Second: %s\n"
						+ "Write Min: %s\n"
						+ "Write Max: %s\n"
						...
						, SizeHelper.getSize(GenericDataCreator.OVERALL_WRITE_RATE.getMeanRate())
						, SizeHelper.getSize(writeRateBytes.getMin())
						, SizeHelper.getSize(writeRateBytes.getMax())
						...
						, SizeHelper.getSize(GenericDataCreator.OVERALL_READ_RATE.getMeanRate())
						, SizeHelper.getSize(readRateBytes.getMin())
						, SizeHelper.getSize(readRateBytes.getMax())
					));
