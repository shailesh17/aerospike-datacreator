...
public enum DataType {
	...
	PICK (null) {
		public Bin create(String name, Config.Bin binConfig) {
			...
			int index = RandomUtils.nextInt(0,binConfig.values.length);
			Object item = binConfig.values[index];
			
			if ( STRING == binConfig.elementType ) {
				return new Bin(name, item.toString());
			} else if ( DOUBLE == binConfig.elementType ) {
				return new Bin(name, Double.valueOf(item.toString()));
			} else {
				return new Bin(name, Long.valueOf(item.toString()));
			}
		}
	},
	...
	private DataType(String dictionaryPath) {
		if ( dictionaryPath != null ) {
			try (InputStream inputStream = DataType.class.getResourceAsStream(dictionaryPath);
					BufferedReader reader = new BufferedReader(new InputStreamReader(inputStream));) {
				
				dictionary = (String[])reader.lines().toArray(String[]::new);			
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
