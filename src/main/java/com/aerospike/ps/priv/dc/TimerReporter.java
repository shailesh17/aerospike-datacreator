...
public class TimerReporter implements Runnable {
	...
	public void printMeters() {
		...
		Map<String, Meter> meters = GenericDataCreator.metrics.getMeters();
		...
		for ( String name : meters.keySet() ) {
			...
			infoFormatter.format("%tT"
					...
					, name
					, meter.getCount()
					, SizeHelper.getSize(meter.getMeanRate())
					, SizeHelper.getSize(meter.getOneMinuteRate())
					, SizeHelper.getSize(meter.getFiveMinuteRate())
					, SizeHelper.getSize(meter.getFifteenMinuteRate())
